...
describe("SpendingLimits", function () {
  ...
  describe("Setting Limits", function () {
    ...
    it("Should reject invalid limit hierarchy", async function () {
      ...
      await expect(
        limits.connect(user).setLimits(daily, weekly, monthly)
      ).to.be.revertedWith("Weekly limit must be >= daily limit");
    });
    ...
    it("Should reject invalid monthly limit", async function () {
      ...
      await expect(
        limits.connect(user).setLimits(daily, weekly, monthly)
      ).to.be.revertedWith("Monthly limit must be >= weekly limit");
    });
    ...
    it("Should allow updating limits", async function () {
      ...
      // Set initial limits
      await limits.connect(user).setLimits(
  ...
  describe("Spender Management", function () {
    ...
    it("Should reject zero address as spender", async function () {
      ...
      await expect(
        limits.connect(user).approveSpender(ethers.ZeroAddress)
  ...
  describe("Recording Spending", function () {
    it("Should allow approved spender to record spending", async function () {
      ...
      await expect(limits.connect(spender).recordSpending(user.address, amount))
        .to.emit(limits, "SpendingRecorded");
    ...
    it("Should reject spending from unapproved spender", async function () {
      ...
      await expect(
        limits.connect(spender).recordSpending(user.address, amount)
    ...
    it("Should reject spending when limits not active", async function () {
      ...
      await expect(
        limits.connect(spender).recordSpending(user.address, amount)
      ).to.be.revertedWith("Spending limits not active");
    });
  ...
  describe("Limit Resets", function () {
    it("Should reset daily limit after 24 hours", async function () {
      ...
      await expect(
        limits.connect(spender).recordSpending(user.address, ethers.parseEther("10"))
      ).to.not.be.reverted;
    ...
    it("Should reset monthly limit after 30 days", async function () {
      ...
      // Should be able to spend again
      await expect(
        limits.connect(spender).recordSpending(user.address, ethers.parseEther("100"))
      ).to.not.be.reverted;
    });
  });
