...
describe("X402Registry", function () {
  ...
  describe("Service Management", function () {
    async function registerServiceFixture() {
      const { registry, owner, provider1, provider2, user } = await loadFixture(deployRegistryFixture);
    ...
    it("Should allow provider to update service", async function () {
      ...
      ).to.emit(registry, "ServiceUpdated").withArgs(serviceId);
    ...
    it("Should not allow non-provider to update service", async function () {
      ...
      await expect(
        registry.connect(provider2).updateService(
          serviceId,
          "Hacked",
    ...
    it("Should allow provider to deactivate service", async function () {
      ...
      await expect(
        registry.connect(provider1).deactivateService(serviceId)
      ).to.emit(registry, "ServiceDeactivated").withArgs(serviceId);
    ...
    it("Should allow owner to deactivate any service", async function () {
      ...
      await expect(
        registry.connect(owner).deactivateService(serviceId)
      ).to.emit(registry, "ServiceDeactivated").withArgs(serviceId);
    ...
    it("Should allow provider to reactivate service", async function () {
      ...
      // Reactivate
      await expect(
        registry.connect(provider1).activateService(serviceId)
      ).to.emit(registry, "ServiceActivated").withArgs(serviceId);
  ...
  describe("Pricing Tiers", function () {
    ...
    it("Should allow multiple pricing tiers", async function () {
      ...
      // Add Basic tier
      await registry.connect(provider1).addPricingTier(
        serviceId,
        "Basic",
        ethers.parseEther("10"),
        100,
    ...
    it("Should not allow non-provider to add pricing tiers", async function () {
      ...
      await expect(
        registry.connect(provider2).addPricingTier(
          serviceId,
          "Malicious",
          ethers.parseEther("1"),
          1000000,
  ...
  describe("Service Verification", function () {
    ...
    it("Should allow owner to verify service", async function () {
      ...
      await expect(
        registry.connect(owner).verifyService(serviceId)
      ).to.emit(registry, "ServiceVerified").withArgs(serviceId);
    ...
    it("Should reject verifying non-existent service", async function () {
      ...
      await expect(
        registry.connect(owner).verifyService(fakeServiceId)
  ...
  describe("Request Recording", function () {
    ...
    it("Should record service requests", async function () {
      ...
      await expect(
        registry.connect(user).recordRequest(serviceId, amount)
      ).to.emit(registry, "RequestRecorded").withArgs(serviceId, amount);
