...
describe('Jarvis402Token', function () {
  ...
  describe('Minting', function () {
    ...
    it('Should not allow minting beyond max supply', async function () {
      ...
      await expect(token.connect(owner).mint(user1.address, excessAmount)).to.be.revertedWith(
        'Exceeds max supply'
      );
    });
  ...
  describe('Reward Distribution', function () {
    it('Should allow authorized distributor to distribute rewards', async function () {
      ...
      await expect(token.connect(distributor).distributeReward(user1.address, rewardAmount, reason))
    ...
    it('Should not allow unauthorized to distribute rewards', async function () {
      ...
      await expect(
        token
          .connect(user1)
          .distributeReward(user2.address, ethers.parseEther('50'), 'Unauthorized')
    ...
    it('Should track total rewards earned', async function () {
      ...
      await token
        .connect(distributor)
        .distributeReward(user1.address, ethers.parseEther('10'), 'Reward 1');
      await token
        .connect(distributor)
        .distributeReward(user1.address, ethers.parseEther('20'), 'Reward 2');
      await token
        .connect(distributor)
        .distributeReward(user1.address, ethers.parseEther('30'), 'Reward 3');
  ...
  describe('Service Provider Rewards', function () {
    it('Should reward service providers', async function () {
      ...
      await expect(token.connect(distributor).rewardServiceProvider(user1.address))
        .to.emit(token, 'RewardDistributed')
  ...
  describe('Payment Rewards', function () {
    it('Should reward payment completion', async function () {
      ...
      await expect(token.connect(distributor).rewardPayment(user1.address))
        .to.emit(token, 'RewardDistributed')
  ...
  describe('Staking', function () {
    ...
    it('Should reject staking with insufficient balance', async function () {
      ...
      await expect(token.connect(user1).stake(ethers.parseEther('1000'))).to.be.revertedWith(
    ...
    it('Should reject staking zero amount', async function () {
      ...
      await expect(token.connect(owner).stake(0)).to.be.revertedWith('Cannot stake 0');
    });
    ...
    it('Should reject unstaking more than staked', async function () {
      ...
      await expect(token.connect(owner).unstake(ethers.parseEther('2000'))).to.be.revertedWith(
  ...
  describe('Token Transfer', function () {
    ...
    it('Should not transfer more than balance', async function () {
      ...
      await expect(token.connect(user1).transfer(user2.address, ethers.parseEther('1000'))).to.be
        .reverted;
    });
  });
});
